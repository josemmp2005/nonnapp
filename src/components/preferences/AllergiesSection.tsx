    ...
    <section className="relative bg-white dark:bg-surface-dark p-6 rounded-2xl border border-ink/10 dark:border-ink-light/10 shadow-sm transition">
      ...
      <div className={locked ? 'opacity-40 pointer-events-none' : ''}>
        <div className="flex items-center gap-3 mb-4 border-b border-ink/5 dark:border-ink-light/10 pb-4">
          ...
          <h2 className="text-lg font-bold text-ink dark:text-[#F8F2E6]">{t('app.preferences.allergies.title')}</h2>
        </div>
        ...
        {useAllergies && (
          <div className="mt-4 animate-in slide-in-from-top-2 fade-in">
            <label htmlFor="allergies-list" className="block text-sm font-medium text-body dark:text-body-dark mb-2">
              {t('app.preferences.allergies.listLabel')}
            </label>
            <textarea
              id="allergies-list"
              className="w-full p-4 rounded-xl border border-ink/15 dark:border-ink-light/15 focus:ring-2 focus:ring-primary focus:border-transparent outline-none h-32 resize-none bg-cream dark:bg-[#221B12] text-ink dark:text-[#F8F2E6]"
