/**
 * Sección de Preferencias: ingredientes que no gustan. Se bloquea si el plan
 * no lo incluye.
 */
      ...
      <div className={locked ? 'opacity-40 pointer-events-none' : ''}>
        <div className="flex items-center gap-3 mb-4 border-b border-[#241B10]/5 dark:border-[#F5E6CD]/10 pb-4">
          <div className="bg-primary/10 p-2 rounded-lg">
            <Ban aria-hidden="true" className="w-5 h-5 text-[#5C4E3A] dark:text-[#A89C86]" />
          </div>
          <h2 className="text-lg font-bold text-[#241B10] dark:text-[#F8F2E6]">{t('app.preferences.disliked.title')}</h2>
        </div>

        <div className="mt-4 animate-in slide-in-from-top-2 fade-in">
          <label htmlFor="disliked-ingredients" className="block text-sm font-medium text-[#3A2E1D] dark:text-[#D4D4D8] mb-2">
            {t('app.preferences.disliked.label')}
          </label>
          <input
            ...
            placeholder={t('app.preferences.disliked.placeholder')}
            value={value}
            onChange={(e) => onChange(e.target.value)}
            disabled={locked}
          />
          <p className="text-xs text-[#6B5D48] dark:text-[#9A8D74] mt-2">
            {t('app.preferences.disliked.hint')}
          </p>
        </div>
      </div>
    </section>
