...
export const PasswordCheckItem: React.FC<Props> = ({ ok, label }) => (
  <div className="flex items-center gap-2 mt-1 px-1">
    <span className="relative w-3 h-3 flex-shrink-0">
      <span
        className={`absolute inset-0 rounded-full border border-ink/20 dark:border-ink-light/15 transition-opacity duration-200 ${ok ? 'opacity-0' : 'opacity-100'}`}
      />
      <Check
    ...
    <span className={`text-xs ${ok ? 'text-green-600' : 'text-muted dark:text-muted-dark'}`}>{label}</span>
  </div>
);
